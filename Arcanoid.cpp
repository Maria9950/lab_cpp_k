...
Bonus Bonus::make(Type t, Vector2f pos)
{
    Bonus b; b.type = t; b.shape.setRadius(BALL_R);
    switch (t) {
    case Type::PadSize:    b.shape.setFillColor(COL_CYAN);    break;
    case Type::BallSpeed:  b.shape.setFillColor(COL_ORANGE);  break;
    case Type::Sticky:     b.shape.setFillColor(COL_PURPLE);  break;
    case Type::BottomSave: b.shape.setFillColor(COL_WHITE);   break;
    case Type::Trajectory: b.shape.setFillColor(COL_HOTPINK); break;
    }
...
bool IndestructibleBlock::onHit(Game&, Ball&, vector<unique_ptr<Bonus>>&) { return false; }
...
bool SpeedUpBlock::onHit(Game&, Ball& b, vector<unique_ptr<Bonus>>&)
{
    b.vel *= 1.15f; b.limit(); dead = true; return true;
}

HealthBlock::HealthBlock(Vector2f p, int h, bool hasB)
{
    hp = h; bonus = hasB; destructible = true;
    shp.setSize(BLOCK_SZ); shp.setOrigin(BLOCK_SZ / 2.f);
...
void Game::initBlocks()
{
    for (int y = 0; y < ROWS; ++y)
        for (int x = 0; x < COLS; ++x)
        {
            ...
            int t = rand() % 10;
            if (t == 0) {
                blocks.emplace_back(make_unique<IndestructibleBlock>(pos));
            }
            else if (t == 1 && rand() % 2 == 0) {
                blocks.emplace_back(make_unique<SpeedUpBlock>(pos));
                ++destroyableLeft;
            }
            else {
                int hp = rand() % 3 + 1;
                bool hasB = (rand() % 3 == 0);
                blocks.emplace_back(make_unique<HealthBlock>(pos, hp, hasB));
                ++destroyableLeft;
            }
        }
}
...
void Game::bonusUpdate()
{
    ...
    bonuses.erase(remove_if(bonuses.begin(), bonuses.end(),
        [](const unique_ptr<Bonus>& p) { return !p || !p->isActive(); }),
        bonuses.end());
}

void Game::applyBonus(Bonus::Type t)
{
    switch (t) {
    case Bonus::Type::PadSize: {
        float w = pad.shp.getSize().x;
        pad.shp.setSize({ std::min(w + PAD_SHR, PAD_MAX_W), PAD_H }); } break;
    case Bonus::Type::BallSpeed: ball.vel *= 1.25f; ball.limit(); break;
    case Bonus::Type::Sticky:    willStick = true;                break;
    case Bonus::Type::BottomSave:bottom = true;                 break;
    case Bonus::Type::Trajectory:ball.randomizeDirection(); ball.limit(); break;
    }
}

void Game::spawnBonus(Vector2f pos)
{
    auto nb = std::make_unique<Bonus>(
        Bonus::make(static_cast<Bonus::Type>(rand() % 5), pos));
    bonuses.emplace_back(std::move(nb));
}
...
void Game::draw()
{
    ...
    for (auto& b : blocks)  win.draw(b->getShape());
    for (auto& p : bonuses) if (p && p->isActive()) win.draw(p->shape);
    win.display();
}

bool HealthBlock::onHit(Game& g, Ball&, vector<unique_ptr<Bonus>>&)
{
    ++g.score; --hp;
    if (hp == 0) {
        g.score++; dead = true;
        if (bonus) {
            Vector2f pos{ shp.getPosition().x - BALL_R,
                          shp.getPosition().y + BLOCK_H / 2 };
            g.spawnBonus(pos);
        }
    }
    else updateCol();
